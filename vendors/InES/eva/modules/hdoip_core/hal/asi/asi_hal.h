...
#define asi_set_src_ip(p, v)            HOI_WR32((p), ASI_OFF_SRC_IP, swap32(v))
#define asi_get_src_ip(p)               swap32(HOI_RD32((p), ASI_OFF_SRC_IP))
#define asi_set_dst_ip(p, v)            HOI_WR32((p), ASI_OFF_DST_IP, swap32(v))
#define asi_get_dst_ip(p)               swap32(HOI_RD32((p), ASI_OFF_DST_IP))
#define asi_set_src_port(p, v)          HOI_WR32((p), ASI_OFF_SRC_PORT, swap16(v))
#define asi_get_src_port(p)             swap16(HOI_RD32((p), ASI_OFF_SRC_PORT))
#define asi_set_dst_port(p, v)          HOI_WR32((p), ASI_OFF_DST_PORT, swap16(v))
#define asi_get_dst_port(p)             swap16(HOI_RD32((p), ASI_OFF_DST_PORT))
