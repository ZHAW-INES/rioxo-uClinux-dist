...
#define asi_set_src_ip(p, st, v)            HOI_WR32((p), asi_reg2off((st), ASI_REG_SRC_IP), swap32(v))
#define asi_get_src_ip(p, st)               swap32(HOI_RD32((p), asi_reg2off((st), ASI_REG_SRC_IP)))
#define asi_set_dst_ip(p, st, v)            HOI_WR32((p), asi_reg2off((st), ASI_REG_DST_IP), swap32(v))
#define asi_get_dst_ip(p, st)               swap32(HOI_RD32((p), asi_reg2off((st), ASI_REG_DST_IP)))
#define asi_set_src_port(p, st, v)          HOI_WR32((p), asi_reg2off((st), ASI_REG_SRC_PORT), swap16(v))
#define asi_get_src_port(p, st)             swap16(HOI_RD32((p), asi_reg2off((st), ASI_REG_SRC_PORT)))
#define asi_set_dst_port(p, st, v)          HOI_WR32((p), asi_reg2off((st), ASI_REG_DST_PORT), swap16(v))
#define asi_get_dst_port(p, st)             swap16(HOI_RD32((p), asi_reg2off((st), ASI_REG_DST_PORT)))
