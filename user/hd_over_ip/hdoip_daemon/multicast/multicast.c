...
void multicast_client_add(int audio_video, t_rtsp_server* server)
{
    ...
    if (audio_video == MEDIA_IS_AUDIO) {
        if (search_client_in_list(server->con.address, &multicast.client_list_audio) == CLIENT_IS_NOT_IN_LIST)
            add_client_to_list(server->con.address, &multicast.client_list_audio, NULL);
    }
    else if (audio_video == MEDIA_IS_VIDEO) {
        if (search_client_in_list(server->con.address, &multicast.client_list_video) == CLIENT_IS_NOT_IN_LIST)
...
int multicast_connection_add(t_rtsp_server* server)
{
    ...
    if (search_client_in_list(server->con.address, &multicast.client_list_start) == CLIENT_IS_NOT_IN_LIST) {
        ...
        ret = multicast.setup_succeeded ? 0 : -1;
                ...
                if (!merge_edids(&edid)) {
                    ...
                    // there's a previous EDID, so compare it to the old one
                        ...
                        while (count_client_list(&multicast.client_list_edid)) {
                            client_ip = get_first_client_and_remove_it_from_list(&multicast.client_list_edid);

                            // only TEARDOWN and close the clients that are not in the start list
                            // as for those the SETUP will fail anyway
                            if (search_client_in_list(client_ip, &multicast.client_list_start) == CLIENT_IS_NOT_IN_LIST)
...
{ 
    ...
    if (search_client_in_list(server->con.address, &multicast.client_list_edid) == CLIENT_IS_NOT_IN_LIST) {
