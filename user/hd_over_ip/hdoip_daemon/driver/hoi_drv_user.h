...
int hoi_drv_aic23b_dac(uint32_t enable, int hp_gain, uint32_t fs, uint32_t width, uint16_t ch_map);
...
int hoi_drv_stsync(uint32_t p);
int hoi_drv_get_fs(uint32_t* p);
int hoi_drv_get_analog_timing(uint32_t* p);
int hoi_drv_get_video_device_id(uint32_t* p);
int hoi_drv_get_audio_device_id(uint32_t* p);
