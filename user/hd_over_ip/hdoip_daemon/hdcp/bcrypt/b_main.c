...
int bcrypt_main(unsigned long *number, char **input_ret) {
  unsigned long sz = 0;
  ...
  //for (; argc > 0; argc--, argv++) {
    ...
    //if
    assignFiles("/mnt/config/key.bfe", &infile, &outfile, &statbuf, &options, key); //enter file to encrypt/decrypt here
     // continue;
