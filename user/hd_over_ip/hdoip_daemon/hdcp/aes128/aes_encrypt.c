...
static unsigned char in[16], out[16], state[4][4];
...
void aes_128(char* schluessel, char* plaintext, char* output){
	size_t i;

		//convert the received string to a unsigned char array
  	BIGD schl_bd;
  	schl_bd = bdNew();
	unsigned char bytes_key[16];
	unsigned char bytes_pt[16];
  	//key
  	bdConvFromHex(schl_bd, schluessel);
  	bdConvToOctets(schl_bd, bytes_key, sizeof(bytes_key));
  	//plaintext
  	bdConvFromHex(schl_bd, plaintext);
  	bdConvToOctets(schl_bd, bytes_pt, sizeof(bytes_pt));
	...
	bdConvFromOctets(schl_bd, out, sizeof(out));
  	//bdPrint(schl_bd, BD_PRINT_NL);
	char temp_h[33];
	bdConvToHex(schl_bd, temp_h, 33);
	//add leading zeros if necessary
  	...
  	temp_h[32] = '\0';
  	for (i=0;i<strlen(temp_h);i++){
    	  output[31-i]=temp_h[strlen(temp_h)-1-i];
  	}
        output[32]='\0';
	bdFree(&schl_bd);
}
